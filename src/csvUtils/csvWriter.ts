...
import { getRandomNumber, getRandomNumberOrNull, getAPairOfDifferentRandomNumbers } from '../utils/helper';
...
class CSVWriter {
  static writeCSV() {
    ...
    for (let i = 0; i <= 80; i++) {
      tweetValues.push(
        `${getRandomNumber(1, 40)},` + `${getRandomNumberOrNull(1, 80)},` + `text-${i},` + `media-${i}\r\n`
      );
    }
